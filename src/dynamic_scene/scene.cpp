...
void Scene::draw_spline_curves(Timeline& timeline)
{
  ...
  for (SceneObject *obj : objects) {
    ...
    for( double t = 0; t <= maxTime; t += 1./nSamplesPerTick ) {
      ...
      /* Zach: This is all complicated stuff to make the spline appear as shifting
    ...
    if (selected) {
      ...
      for(Spline<Vector3D>::KnotCIter k = spline.knots.begin(); k != spline.knots.end(); k++)
      {
        double t = k->first;
        Vector3D P = k->second; // position
        Vector3D T = tangentLengthScale * spline.evaluate( t, 1 ); // tangent
        Vector3D A = tangentLengthScale * spline.evaluate( t, 2 ); // acceleration
        glColor4f( 1., 1., 1., alpha );
