...
void XFormWidget::restoreLastMode()
{
   mode = lastMode;
}
...
void XFormWidget::set_draw_styles(DrawStyle *defaultStyle, DrawStyle *hoveredStyle, DrawStyle *selectedStyle)
{
...
void XFormWidget::directionalTransform( Vector3D& p, vector<int> I, Vector3D c, double x, double y, double dx, double dy, const Matrix4x4& modelViewProj )
{
   ...
   if( mode == Mode::Rotate )
   {
      ...
      Vector2D C( viewport[2]*(d.x+1.)/2., viewport[3]*(1.-(d.y+1.)/2.) );
      Vector2D U = ( A-C ).unit();
      Vector2D V = ( B-C ).unit();
      ...
      if( i == 0 )
      {
         ...
         R(1,0) = 0.; R(1,1) =  cos(m); R(1,2) = sin(m);
         R(2,0) = 0.; R(2,1) = -sin(m); R(2,2) = cos(m);
      }
      else if( i == 1 )
      {
         R(0,0) = cos(m); R(0,1) =  0.; R(0,2) = -sin(m);
         R(1,0) =     0.; R(1,1) =  1.; R(1,2) =  0.;
         R(2,0) = sin(m); R(2,1) =  0.; R(2,2) =  cos(m);
      }
      else if( i == 2 )
      {
         R(0,0) = cos(m); R(0,1) = -sin(m); R(0,2) = 0.;
         R(1,0) = sin(m); R(1,1) =  cos(m); R(1,2) = 0.;
...
StaticScene::SceneObject* XFormWidget::get_static_object()
{
   return nullptr;
}
...
void XFormWidget::drawScaleHandles() const
{
   ...
   for( int i = 0; i < 3; i++ )
   {
      ...
      glVertex3dv( &q000.x ); glVertex3dv( &q001.x ); glVertex3dv( &q011.x ); glVertex3dv( &q010.x );
      glVertex3dv( &q100.x ); glVertex3dv( &q101.x ); glVertex3dv( &q111.x ); glVertex3dv( &q110.x );
      glVertex3dv( &q000.x ); glVertex3dv( &q010.x ); glVertex3dv( &q110.x ); glVertex3dv( &q100.x );
      glVertex3dv( &q001.x ); glVertex3dv( &q011.x ); glVertex3dv( &q111.x ); glVertex3dv( &q101.x );
      glVertex3dv( &q000.x ); glVertex3dv( &q100.x ); glVertex3dv( &q101.x ); glVertex3dv( &q001.x );
      glVertex3dv( &q010.x ); glVertex3dv( &q110.x ); glVertex3dv( &q111.x ); glVertex3dv( &q011.x );
   }
   glEnd();
